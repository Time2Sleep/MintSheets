...
<template>
  <div class="flex gap-2 items-end">
    <BaseInput v-model="input" :label="label" :name="name" class="flex-1" :placeholder="placeholder" />
  ...
  <div v-if="list?.length" class="px-2 text-sm">
    <div ref="scrollableList" class="overflow-y-auto" :style="getStyle">
      <div v-for="(item, index) in list" :key="item" class="my-2 flex gap-4 items-center">
        <BaseButton type="button" class="py-1" @click="removeItem(item)">
          <svg
